...
export const PivotConfiguratorPanel: React.FC<PivotConfiguratorPanelProps> = ({
  ...
  return (
    <>
      <Collapse in={!isConfiguratorCollapsed} timeout={260} easing="cubic-bezier(0.2, 0, 0, 1)" unmountOnExit>
        <Box
          sx={{
            display: 'grid',
      ...
      <Box sx={{ position: 'relative', my: 0.8 }}>
        <Divider />
        <IconButton
          size="small"
          aria-label={isConfiguratorCollapsed ? t('pivot.collapse.ariaExpand') : t('pivot.collapse.ariaCollapse')}
          ...
          sx={(theme) => ({
            ...
            borderRadius: 999,
            '&:hover': {
              bgcolor: theme.palette.mode === 'dark' ? 'rgba(25,118,210,0.2)' : 'rgba(47, 91, 255, 0.08)',
            },
          })}
        >
          <KeyboardArrowUpIcon
            sx={{
              ...
              transform: isConfiguratorCollapsed ? 'rotate(180deg)' : 'rotate(0deg)',
            }}
          />
        </IconButton>
      </Box>
